		...
		$.get(url, folderValue, function (data) {
			if (data.success) {
				document.getParentByClassList(e.currentTarget, "list-group-item", function (result) {
					$(result).after($('<a href="#" onclick="return false" data-folder="' + data.success + '" class="list-group-item selectFolder"><label class="glyphicon glyphicon-folder-close"> ' + data.success + '</label></a>').get(0));
				});
			}
		}, "json");
	});
